...
The following function runs a linear regression in R:

```         
linear_model <- lm(dependent_variable ~ independent_variable1, data = df)
```

where

-   The dependent variable (aka the predicted variable) is on the left side of the `~`

-   Independent variables (aka predictors, covariates, or features) are on the right side of the `~`.

-   Both variables need to be the in the data frame, `df`.

-   `linear_model` is the result of the linear regression
